...
## Objectif
Mettre en place un stage `release` au pipeline GitLab CI/CD pour générer automatiquement les versions et releases GitLab basées sur les commits respectant la convention Conventional Commits, en utilisant semantic-release avec les plugins GitLab.
...
- `@semantic-release/npm` — gestion npm (ici non publié)
...
- Utilise l'image `node:22` pour exécuter semantic-release
- Injecte `CI_JOB_TOKEN` automatique pour authentifier les push vers GitLab

### 4) Comportement du release
...
5. **Git** : commit + push ces fichiers sur main
...
Les images publiées dans le stage `build` gardent les tags basés sur le SHA du commit.  
Option future : retaguer les images avec la version sémantique après release (ex: `registry.gitlab.com/groupe/proj:1.2.3`).
...
Le release génère une version sémantique et une release GitLab, mais il ne retagge pas automatiquement les images Docker avec cette version. Cette étape pourrait être ajoutée via un job supplémentaire `retag_images` après `release_angular`.

## Résultat attendu atteint

- semantic-release est configuré pour Angular (dépendances, .releaserc.json).
...
- Les versions sont synchronisées dans package.json via semantic-release.
...
4. Vérifier la génération de la release GitLab et du changelog.
