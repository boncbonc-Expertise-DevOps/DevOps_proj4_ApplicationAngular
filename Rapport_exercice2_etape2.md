# Rapport exercice 2 — Étape 2 (Application Angular)
...
Dans ce repository Angular, seul `angular_tests` s’exécute grâce aux règles `rules: exists`.
...
Dans ce repository Angular, `angular_lint` est le job concerné.
...
- que dans un repo GitLab unique (cas actuel Angular).

### 5) Cache des dépendances
Pour accélérer les builds :
- cache npm activé (`.npm/`)
- cache `node_modules/`
- clé de cache basée sur `package-lock.json`

### 6) Rapports de tests GitLab (JUnit)
Le pipeline publie :
- artifacts : `test-results/`, `reports/`, `coverage/`
- rapports JUnit GitLab : `test-results/**/*.xml`

Les fichiers XML sont générés par Karma/Jasmine puis agrégés par le script dans `test-results/`.
...
- `archive.zip` : archive des chemins declares dans `artifacts:paths` (par exemple `test-results/`, `reports/`, `coverage/`).
...
- `bash ./run-tests.sh`
- Résultat : succès des tests Angular + génération/agrégation des XML JUnit
